...
- **Ring pops.** The first 500 kicks earn one; every 1,000 after that earns another (500, 1,500, 2,500 ...). The page works out how many each girl has *earned* from her total. The leaderboard has a **Ring pops** column: given *of* earned, what is still to give, and a **Gave one** button while she is owed one, which records a ring pop handed over at `pop:<player>` as an absolute count (a tap sent twice cannot hand out a phantom one). **Adjust**, under her kicks, edits both numbers. A tap changes that cell and nothing else: the leaderboard is sorted by kicks, so nothing re-sorts; the button leaves an invisible placeholder so the row keeps its height; and there is no confirmation box at the top to shove the table down. Parents see how many kicks to the next one under the girl's total.
