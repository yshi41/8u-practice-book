...
- **Ring pops.** The first 500 kicks earn one; every 1,000 after that earns another (500, 1,500, 2,500 ...). The page works out how many each girl has *earned* from her total and says so on her leaderboard row ("2 ring pops earned · one to give"). **Gave one** sits on that row while she is owed one and records it handed over, stored at `pop:<player>` as an absolute count so a tap sent twice cannot hand out a phantom one. The count given can be corrected under **Adjust**, next to her kicks. A tap changes that row's own words and nothing else: the leaderboard is sorted by kicks, so nothing re-sorts, the button leaves an invisible placeholder so the row keeps its height, and there is no confirmation box at the top to shove the table down. Parents see how many kicks to the next one under the girl's total.
